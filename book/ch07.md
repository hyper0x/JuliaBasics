...
由于上述的特定类型都是`Ptr`类型的子类型，所以我们说原语类型依然可以因参数化而成为当下的类型族群之首。并且，参数化的原语类型依然具有非转化特性。
...
如果要解释位类型的值是怎样应用在类型参数中的话，我觉得最好的案例莫过于我们在前面见过很多次的类型`Array`。`Array`是一个参数化类型，它的全名是`Array{T,N}`。其中的类型参数`T`用于确定数组的元素类型，而类型参数`N`则用于确定数组的维度。也就是说，数组的维度与它的元素类型一样都会被写入到它的类型字面量中。原则上，`N`的取值可以是任何`Int64`类型的值（如果在 32 位的计算机系统中，那么就是任何`Int32`类型的值）。但在实际应用中，`N`的值肯定不能是负数。
...
另外，我们需要谨记“某个类型的类型”与“某个类型的超类型”这两个概念之间的不同。虽然它们要解答的都是类型的归类问题，但不同的是：“某个类型的类型”说的是这个类型的先天归属，就像在说一个人的性别或者血统；而“某个类型的超类型”说的是一个类型的后天分类，就像在说一个人的职业或者国籍。一个值（别忘了类型也是一种值）从被编写或被构造的那时起就会有一个类型，而一个类型继承自哪一个超类型却需要我们通过编写代码来指定。显然，这是两个不同维度的归类问题。我们可以说这两者是正交的。一个类型既会有它隶属的类型，也会有它从属的超类型（最起码有`Any`），并且各自独立、互不干扰。
...
可以看到，有名元组同样由圆括号包裹，也同样以英文逗号分隔其中的多个元素值。但与普通的元组不同的是，在有名元组中的每一个元素值的左侧，都有一个代表了元素名称的标识符和一个等号。这种表示形式与对变量的赋值极其相似。而且这两者的含义也基本相同，即：把一个值与一个标识符绑定在一起。但是，它们的作用域是不同的。虽然有名元组中的元素值也可以通过其名称来访问，但这些名称仅在其所属元组的上下文中可用。例如：
...
`Symbol`本来是元编程中的一个概念，它的值用于表示对变量的访问。在有名元组的上下文中，其值的含义就是指代某个元素值的名称，而在`:`后面的就是那个名称。又由于这里的`Symbol`类型值与索引号的作用是相同的，因此前述表达式的求值结果就是与`reg_year`对应的那个元素值。
...
其中的`Vararg{String}`就是那个特殊的类型参数值。它是`Vararg{String,N} where N`的简写形式。而`Vararg`是一个直接继承自`Any`的抽象类型，同时也是一个参数化类型。它拥有两个类型参数，其占位符分别是`T`和`N`。因此，类型`Vararg{T,N}`表达的就是`N`个`T`类型的参数。若放到元组类型的上下文中，它则表示该元组类型的所有实例都要有`N`个`T`类型的元素值。因为元组中元素值的数量总是与其类型中类型参数的数量保持一致。
...
我们在这一章主要讨论了参数化类型。参数化是 Julia 类型系统中的一个非常重要且强大的特性。它允许一个类型自身拥有参数，以使其可以代表一个完整的类型族群。在很多时候，参数化类型就相当于一种对数据结构的泛化定义，因此我们也可以称之为泛型类型。我们之前讲过的抽象类型、原语类型和复合类型都可以被参数化。其中，参数化复合类型最常用，但抽象类型的参数化意义更大。
