...
另外，我们需要谨记“某个类型的类型”与“某个类型的超类型”这两个概念之间的不同。虽然它们要解答的都是类型的归类问题，但不同的是：“某个类型的类型”说的是这个类型的先天归属，就像在说一个人的性别或者民族；而“某个类型的超类型”说的是一个类型的后天分类，就像在说一个人的职业或者国籍。一个值（别忘了类型也是一种值）从被编写或被构造的那时起就会有一个类型，而一个类型继承自哪一个超类型却需要我们通过编写代码来指定。显然，这是两个不同维度的归类问题。我们可以说这两者是正交的。一个类型既会有它隶属的类型，也会有它从属的超类型（最起码有`Any`），并且各自独立、互不干扰。
