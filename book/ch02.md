...
很显然，这两个快捷键分别用于从两个不同的方向在这个历史文件中搜索指定的内容。只要我们在这种模式下输入一些字符，它就可以实时地为我们查找相关的历史表达式或命令。此时，光标会自动地定位在匹配内容的开始处。并且，当我们再次按下`Ctrl+s`或`Ctrl+r`时，它还会立即为我们显示下一个或上一个匹配的内容。
