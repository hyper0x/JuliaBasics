...
关于更多的快捷键，你可以查阅 Julia 语言的[官方文档](https://docs.julialang.org/en/v1/stdlib/REPL/#Key-bindings-1)。
...
顺便说一句，以`/Applications/Julia-1.1.app`开头的仓库目录只会在 macOS 操作系统下出现。如果是在 Linux 操作系统中，这里的仓库目录应该还会有`/usr/local/julia`和`/usr/share/julia`。
